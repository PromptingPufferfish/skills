...
- For need-style user messages, prefer `POST /skill/needs/auto-publish` (auto-publish default path).
...
### 4. Conversational Shortcuts (Auto-Publish by Default)
...
- On a clear need message, auto-publish in the same turn:
  - call `POST /skill/needs/auto-publish`,
  - store returned `posting_id` in `recent_need_context`,
  - acknowledge publish completion to the user.
...
- If a short confirmation arrives and context is fresh:
  - Reuse `need_text` and call `POST /skill/needs/auto-publish`.
  - If deduped, return existing `posting_id` and do not republish.
